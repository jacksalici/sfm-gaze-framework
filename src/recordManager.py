# Code modified from Meta library.
#
...
def download_all_files():
    ...
    base_path = pathlib.Path(config["aria_recordings"]["vrs_glob"])
#ticsync_server_files = dict{shared_session_id : [date, device_serial, rec_uuid]}
...
def main():
    ...
    if args.list_recordings:
        list_files(True)
        if args.download_recordings:
            download_all_files()    
            
    if args.shared_session_id:
        download_files(args.shared_session_id)
